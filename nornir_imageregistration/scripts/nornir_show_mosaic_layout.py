...
def ParseArgs(ExecArgs=None):
    """Parse command-line arguments for the show_mosaic_layout script. Returns (namespace, unknown)."""
    if ExecArgs is None:
        ExecArgs = sys.argv
...
def Execute(ExecArgs=None):
    """Run the show_mosaic_layout script with the given (or default) command-line args."""
    if ExecArgs is None:
        ExecArgs = sys.argv[1:]
    ...
    if downsample is None:
        try:
            ...
            print(
                "downsample not specified and could not be determined from filename.  Please pass the downsample parameter.")
            return

    initial_tiles = nornir_imageregistration.mosaic_tileset.CreateFromMosaic(mosaic, image_folder=TilesDir,
                                                                             image_to_source_space_scale=downsample)

    (distinct_overlaps, new_overlaps, updated_overlaps, removed_overlap_IDs,
     non_overlapping_IDs) = nornir_imageregistration.arrange_mosaic.GenerateTileOverlaps(initial_tiles,
