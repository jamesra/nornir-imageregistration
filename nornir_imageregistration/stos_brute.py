...
def ScoreOneAngle(imFixed_original: NDArray, imWarped_original: NDArray,
    ...
    if angle != 0:
        ...
        imWarped = scipy.ndimage.rotate(imWarped.astype(np.float32, copy=False), axes=(0, 1), angle=-angle,
                                        cval=np.nan).astype(imWarped.dtype,
                                                            copy=False)  # Numpy cannot rotate float16 images
