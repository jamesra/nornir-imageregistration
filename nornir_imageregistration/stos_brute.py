...
def SliceToSliceBruteForce(FixedImageInput,
    ...
    BestMatch = FindBestAngle(imFixed, imWarped, AngleSearchRange, MinOverlap=MinOverlap, SingleThread=SingleThread, use_cluster=Cluster, use_cp=use_cp)
    ...
    if TestFlip:
        ...
        BestMatchFlipped = FindBestAngle(imFixed, imWarpedFlipped, AngleSearchRange, MinOverlap=MinOverlap, SingleThread=SingleThread, use_cluster=Cluster, use_cp=use_cp)
    ...
    if not UserDefinedAngleSearchRange:
        BestRefinedMatch = FindBestAngle(imFixed, imWarped, [(x * 0.1) + BestMatch.angle - 1.9 for x in range(0, 18)], MinOverlap=MinOverlap, SingleThread=SingleThread, use_cluster=Cluster, use_cp=use_cp)
        ...
        if len(AngleSearchRange) > 2:
            ...
            BestRefinedMatch = FindBestAngle(imFixed, imWarped, [(x * stepsize) + below for x in range(1, nSteps)], MinOverlap=MinOverlap, SingleThread=SingleThread, use_cp=use_cp)
            BestRefinedMatch.flippedud = IsFlipped
        else:
...
def FindBestAngle(imFixed: NDArray, imWarped: NDArray, AngleList: list[float] | None, MinOverlap: float = 0.75, SingleThread: bool = False, use_cluster: bool = False, use_cp: bool = False):
...
#    LargePaddedFixed = PadImageForPhaseCorrelation(imFixed, MaxOffset=0.1)
    ...
    if not (use_cluster or SingleThread):
        ...
        SharedPaddedFixed = PaddedFixed.astype(np.float32, copy=False) if not use_cp else cp.array(PaddedFixed, np.float32)
        SharedWarped = imWarped.astype(np.float32, copy=False) if not use_cp else cp.array(imWarped, np.float32)
    ...
    for i, theta in enumerate(AngleList): 
        if SingleThread:
            record = ScoreOneAngle(SharedPaddedFixed, SharedWarped, fixed_shape,
                                    ...
                                    MinOverlap=MinOverlap, use_cp=use_cp)
