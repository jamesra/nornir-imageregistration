...
def _IterateOverlappingTiles(list_tiles, min_overlap: float = None, exclude_diagonal_overlaps: bool = False):
    ...
    for (A, B) in rset.EnumerateOverlapping():
        if min_overlap is None:
            ...
            if exclude_diagonal_overlaps and nornir_imageregistration.Rectangle.is_diagonal_overlap(list_rects[A],
                                                                                                    list_rects[B]):
                continue
...
def CreateTileOverlaps(list_tiles,
                       image_to_source_space_scale: float = 1.0,
...
ea that must overlap between tiles
    ...
    for (A, B) in _IterateOverlappingTiles(list_tiles, min_overlap=min_overlap,
                                           exclude_diagonal_overlaps=exclude_diagonal_overlaps):
        overlap_data = TileOverlap(A, B, image_to_source_space_scale=image_to_source_space_scale,
                                   inter_tile_distance_scale=inter_tile_distance_scale)
...
class TileOverlap(object):
    ...
    def overlap(self):
        ...
        if self._overlap is None:
            if self.scaled_overlapping_source_rect_A is None or self.scaled_overlapping_source_rect_B is None:
                ...
                self._overlap = nornir_imageregistration.Rectangle.overlap_normalized(self.A.FixedBoundingBox,
                                                                                      self.B.FixedBoundingBox)
    ...
    def __init__(self,
                 ...
                 image_to_source_space_scale: float | None,
                 inter_tile_distance_scale: float = 1.0):
    ...
    def Calculate_Overlapping_Regions(A: nornir_imageregistration.Tile, B: nornir_imageregistration.Tile,
        ...
        if inter_tile_distance_scale != 1.0:
            ...
            moved_B_target_bbox = nornir_imageregistration.Rectangle.CreateFromCenterPointAndArea(
                new_B_target_bbox_center, B_target_bbox.Dimensions)

            moved_A_target_bbox = nornir_imageregistration.Rectangle.CreateFromCenterPointAndArea(
                new_A_target_bbox_center, A_target_bbox.Dimensions)
        ...
        overlapping_rect_B = nornir_imageregistration.Rectangle.translate(overlapping_rect_B,
                                                                          original_offset - OffsetAdjustment)
    ...
    def scale_overlapping_rects(overlapping_rect_A, overlapping_rect_B, scalar):
        ...
        downsampled_overlapping_rect_B = nornir_imageregistration.Rectangle.SafeRound(
            nornir_imageregistration.Rectangle.CreateFromBounds(overlapping_rect_B.ToArray() * scalar))
        ...
        downsampled_overlapping_rect_B = nornir_imageregistration.Rectangle.CreateFromPointAndArea(
            downsampled_overlapping_rect_B.BottomLeft, downsampled_overlapping_rect_A.Size)
