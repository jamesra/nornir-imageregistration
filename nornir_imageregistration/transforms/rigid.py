...
class RigidNoRotation_GPU(base.ITransform, base.ITransformScaling, base.ITransformTranslation,
                          DefaultTransformChangeEvents):
    '''This class is legacy and probably needs a deprecation warning'''
    ...
    def TranslateFixed(self, offset: NDArray[float]):
        '''Translate all fixed points by the specified amount'''
        self.target_offset = self.target_offset + cp.array(offset)
        self.OnTransformChanged()
    ...
    def __init__(self, target_offset=tuple[float, float] | list[float, float] | NDArray[float],
        ...
        :param Rectangle MappedBoundingBox: Optional, the boundaries of points expected to be mapped.  Used for informational purposes only.
        '''
        super(RigidNoRotation_GPU, self).__init__()
        ...
        self.target_offset = nornir_imageregistration.EnsurePointsAre1DCuPyArray(target_offset)
        self.source_space_center_of_rotation = nornir_imageregistration.EnsurePointsAre1DCuPyArray(
            source_rotation_center)
        self._angle = angle  # type: float
    ...
    def __setstate__(self, dictionary):
        ...
        self.target_offset = cp.asarray((self.target_offset[0], self.target_offset[1]), dtype=np.float32)
        self.source_space_center_of_rotation = cp.asarray((self.source_space_center_of_rotation[0],
                                                           self.source_space_center_of_rotation[1]), dtype=np.float32)
    ...
    def Transform(self, points, return_cp: bool = False, **kwargs):
        ...
        transformed = points + self.target_offset
        if return_cp:
            return transformed
        else:
    ...
    def InverseTransform(self, points, return_cp: bool = False, **kwargs):
        ...
        points = nornir_imageregistration.EnsurePointsAre2DCuPyArray(points)
        itransformed = points - self.target_offset
        if return_cp:
            return itransformed
        else:
            return itransformed.get()


class Rigid_GPU(base.ITransformSourceRotation, RigidNoRotation_GPU):
    '''
    Applies a rotation+translation transform
    '''

    def __getstate__(self):
        data = super(Rigid_GPU, self).__getstate__()
        data['flip_ud'] = self.flip_ud
        return data

    def __setstate__(self, dictionary):
        super(Rigid_GPU, self).__setstate__(dictionary)
        self.flip_ud = dictionary['flip_ud']
        self.forward_rotation_matrix = nornir_imageregistration.transforms.utils.RotationMatrix_GPU(self.angle)
        self.inverse_rotation_matrix = nornir_imageregistration.transforms.utils.RotationMatrix_GPU(-self.angle)
    ...
    def __init__(self, target_offset: tuple[float, float] | list[float, float] | NDArray,
        ...
        :param Rectangle MappedBoundingBox: Optional, the boundaries of points expected to be mapped.  Used for informational purposes only.
        '''
        super(Rigid_GPU, self).__init__(target_offset, source_rotation_center, angle)
    ...
    def update_rotation_matrix(self):
        ...
        if self.flip_ud:
            flip_y_matrix = nornir_imageregistration.transforms.utils.FlipMatrixY_GPU()
            self.forward_rotation_matrix = cp.matmul(flip_y_matrix, self.forward_rotation_matrix)
            self.inverse_rotation_matrix = cp.matmul(flip_y_matrix, self.inverse_rotation_matrix)
    ...
    def Transform(self, points, return_cp: bool = False, **kwargs):
        ...
        centered_points = points - self.source_space_center_of_rotation
        # centered_points = np.transpose(centered_points)
        centered_points = cp.hstack((centered_points, cp.ones((numPoints, 1))))

        # centered_rotated_points = (self.forward_rotation_matrix @ centered_points.T).T
        centered_rotated_points = cp.transpose(cp.matmul(self.forward_rotation_matrix, cp.transpose(centered_points)))
        # centered_rotated_points = np.transpose(centered_rotated_points)
        centered_rotated_points = centered_rotated_points[:, 0:2]

        rotated_points = centered_rotated_points + self.source_space_center_of_rotation
        output_points = rotated_points + self.target_offset
        transformed = cp.around(output_points, nornir_imageregistration.RoundingPrecision(output_points.get().dtype))
        if return_cp:
            return transformed
        else:
            return transformed.get()
    ...
    def InverseTransform(self, points, return_cp: bool = False, **kwargs):
        ...
        # centered_points = np.transpose(centered_points)
        centered_points = cp.hstack((centered_points, cp.zeros((numPoints, 1))))
        # rotated_points = centered_points @ self.inverse_rotation_matrix
        # rotated_points = (self.inverse_rotation_matrix @ centered_points.T).T
        rotated_points = cp.transpose(cp.matmul(self.inverse_rotation_matrix, cp.transpose(centered_points)))
        # rotated_points = np.transpose(rotated_points)
        rotated_points = rotated_points[:, 0:2]

        output_points = rotated_points + self.source_space_center_of_rotation
        itransformed = cp.around(output_points, nornir_imageregistration.RoundingPrecision(output_points.get().dtype))
        if return_cp:
            return itransformed
        else:
            return itransformed.get()
    ...
    def RotateSourcePoints(self, rangle: float, rotation_center: NDArray[float] | None):
        ...
        if rotation_center is not None:
            self.source_space_center_of_rotation = cp.array(rotation_center)
...
class CenteredSimilarity2DTransform_GPU(Rigid_GPU, base.ITransformRelativeScaling):
    '''
    Applies a scale+rotation+translation transform
    '''

    def __getstate__(self):
        odict = super(CenteredSimilarity2DTransform_GPU, self).__getstate__()
        odict['_scalar'] = self._scalar
        return odict

    def __setstate__(self, dictionary):
        super(CenteredSimilarity2DTransform_GPU, self).__setstate__(dictionary)
        self._scalar = dictionary['_scalar']
    ...
    def __init__(self, target_offset: tuple[float, float] | list[float, float] | NDArray,
        ...
        :param Rectangle MappedBoundingBox: Optional, the boundaries of points expected to be mapped.  Used for informational purposes only.
        '''
        super(CenteredSimilarity2DTransform_GPU, self).__init__(target_offset, source_rotation_center, angle, flip_ud)
    ...
    def Transform(self, points, return_cp: bool = False, **kwargs):
        ...
        centered_points = points - self.source_space_center_of_rotation
        # centered_points = np.transpose(centered_points)
        centered_points = cp.hstack(
            (centered_points, cp.ones((numPoints, 1))))  # Add a row so we can multiply the matrix
        ...
        rotated_points = centered_rotated_points + self.source_space_center_of_rotation
        output_points = rotated_points + self.target_offset
        transformed = cp.asarray(output_points)
        if return_cp:
            return transformed
        else:
            return transformed.get()
    ...
    def InverseTransform(self, points, return_cp: bool = False, **kwargs):
        ...
        output_points = rotated_points + self.source_space_center_of_rotation
        itransformed = cp.asarray(output_points)
        if return_cp:
            return itransformed
        else:
            return itransformed.get()
