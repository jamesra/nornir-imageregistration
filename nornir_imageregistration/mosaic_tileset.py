...
def CreateFromMosaic(mosaic, image_folder:str, image_to_source_space_scale:float):
    '''
    :param mosaic Mosaic: The mosaic we are pulling transforms and image filenames from
    :param str image_folder: The folder containing the image files 
    :param float source_space_scale: Scalar for the transform source space coordinates.  Must match the change in scale of input images relative to the transform source space coordinates.  So if downsampled by
    ...
    for i, item in enumerate(mosaic.ImageToTransform.items()):
        ...
        tile = nornir_imageregistration.tile.Tile(transfrom, filename, image_to_source_space_scale=image_to_source_space_scale, ID=tile_number)
...
def Create(transforms, imagepaths, image_to_source_space_scale:float):
        ...
        for i, t in enumerate(transforms):
            ...
            tile = nornir_imageregistration.tile.Tile(t, imagepaths[i], image_to_source_space_scale=image_to_source_space_scale, ID=tile_number)
            obj[tile_number] = tile

        return obj

class MosaicTileset(dict):
    '''
    ...
    def TargetBoundingBox(self) -> nornir_imageregistration.Rectangle:
        ...
        transforms = self.values()
        self._target_space_bounding_box = nornir_imageregistration.Rectangle.Union([t.TargetSpaceBoundingBox for t in transforms])
        return self._target_space_bounding_box
    ...
    def TranslateTargetSpace(self, offset):
        ...
        if self._target_space_bounding_box is not None:
            self._target_space_bounding_box = nornir_imageregistration.Rectangle.translate(self._target_space_bounding_box, offset)
    ...
    def GenerateOptimizedTiles(self, tile_dims=None, max_temp_image_area=None, usecluster=True, target_space_scale=None, source_space_scale=None):
        '''
