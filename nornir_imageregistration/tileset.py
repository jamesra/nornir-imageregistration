...
def __DetermineTransformScale(transform, imageSize):
    ...
    if nornir_imageregistration.ApproxEqual(imageSize[0], height, epsilon=1.1) and nornir_imageregistration.ApproxEqual(
            imageSize[1], width, epsilon=1.1):
        return 1.0
    else:
...
def MostCommonScalar(transforms, imagepaths):
    ...
    raise DeprecationWarning(
        "Examine why we are determining scale from a set of images.  Attempt to replace with mosaic_tileset's attribute for image to source space scale")
    scales = []
...
def __CorrectBrightfieldShadingOneImage(input_fullpath, output_fullpath, imagescalar, bpp):
    ...
    image = image.astype(nornir_imageregistration.default_image_dtype(), copy=False) / max_pixel_value
