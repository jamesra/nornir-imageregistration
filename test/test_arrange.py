...
def ShowTilesWithOffset(test, layout_obj, tiles_list, TileA_ID: int, TileB_ID: int, output_dir: str, filename: str,
                        image_to_source_space_scale: float, openwindow: bool, weight: float = None):
    ...
    mosaic_set = nornir_imageregistration.mosaic_tileset.MosaicTileset(
...
class TestMosaicArrange(setup_imagetest.TransformTestBase, setup_imagetest.PickleHelper):
    ...
    def __CheckAllOffsetsPresent(self, layout, expected_offsets):
        ...
        for offset in expected_offsets:
            self.assertTrue(layout.ContainsOffset(offset),
                            "{0} should have an offset in the layout but does not".format(offset))
    ...
    def CalculateOffsetsForTiles(self, tile_offsets_to_update, all_tile_offsets, excess_scalar,
                                 image_to_source_space_scale, existing_layout, use_feature_score: bool):

        translate_layout = arrange._FindTileOffsets(tile_offsets_to_update, excess_scalar, image_to_source_space_scale,
                                                    existing_layout, use_feature_score=use_feature_score)
    ...
    def ArrangeMosaicDirect(self, config: nornir_imageregistration.settings.TranslateSettings, mosaicFilePath,
        ...
        for iPass in range(0, config.max_translate_iterations):
            (distinct_overlaps, new_overlaps, updated_overlaps, removed_overlap_IDs,
             non_overlapping_IDs) = arrange.GenerateTileOverlaps(mosaic_tileset,
            ...
            if translated_layout is not None:
                ...
                for ID in non_overlapping_IDs:
                    self.assertTrue(translated_layout.nodes[ID].ConnectedIDs.shape[0] == 0,
            ...
            if not config.feature_score_calculations_required:
                ...
                Scores.sort()
                h = nornir_shared.histogram.Histogram.Init(minVal=np.min(Scores), maxVal=np.max(Scores),
                                                           numBins=int(np.sqrt(len(Scores)) * 10))
                h.Add(Scores)

                # nornir_shared.plot.Histogram(h)
                nornir_shared.plot.Histogram(h, ImageFilename=os.path.join(self.TestOutputPath,
                                                                           "{0:d}pass_FeatureScoreHistogram.png".format(
                                                                               iPass)),
                ...
                for overlap in new_or_updated_overlaps:
                    if overlap.feature_scores[0] >= config.feature_score_threshold and overlap.feature_scores[
                        1] >= config.feature_score_threshold:
            ...
            self.SaveVariable((distinct_overlaps, new_overlaps, updated_overlaps, removed_overlap_IDs, locked_overlaps,
                               overlap_colors),
            ...
            nornir_imageregistration.views.plot_tile_overlaps(new_overlaps + updated_overlaps + locked_overlaps,
                                                              ...
                                                              OutputFilename=os.path.join(self.TestOutputPath,
                                                                                          "pass_{0}_tile_overlaps.svg".format(
                                                                                              iPass)))

            translated_layout = self.ReadOrCreateVariable(
                self.id() + "_{0:d}pass_tile_layout_{1:03d}_{2:03g}".format(iPass, downsample,
                                                                            config.first_pass_excess_scalar),
            ...
            self.PlotLayoutWeightHistogram(scaled_translated_layout,
