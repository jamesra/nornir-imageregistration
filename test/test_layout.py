...
class TestLayoutPosition(setup_imagetest.TestBase):
    ...
    def test_weighted_line(self):
        ...
        RelaxLayout(spring_layout, plotting_output_path=self.TestOutputPath, max_tension_cutoff=max_vector_magnitude,
                    max_iter=100)

        for ID in spring_layout.nodes.keys():
            self.assertTrue(setup_imagetest.array_distance(spring_layout.NetTensionVector(ID)) < max_vector_magnitude,
                            "Node %d should have net tension vector below relax cutoff")
    ...
    def test_uneven_weighted_line(self):
        ...
        for ID in spring_layout.nodes.keys():
            self.assertTrue(
                setup_imagetest.array_distance(spring_layout.WeightedNetTensionVector(ID)) < max_vector_magnitude,
                "Node %d should have net tension vector below relax cutoff")

        self.assertTrue(np.allclose(spring_layout.GetPosition(0) - spring_layout.GetPosition(1), (-8.333, -5),
                                    atol=max_vector_magnitude * 2))  # Since the nodes at the ends both move we expect equilibrium at 8.33 instead of 7.5
        self.assertTrue(np.allclose(spring_layout.GetPosition(0) - spring_layout.GetPosition(2), (8.333, -5),
                                    atol=max_vector_magnitude * 2))
...
class TestLayout(setup_imagetest.TestBase):
    ...
    def do_layout(self, num_cols, num_rows):
        ...
        self.assertTrue(len(merged_layout.nodes) == expected_node_count,
                        f"Expected {expected_node_count} nodes in final layout, got {len(merged_layout.nodes)}")
        # for layout in layout_lists:
    ...
    def test_layout_relax_into_grid_random_offset_uniform_weight(self):
        self._run_layout_relax_into_grid(lambda: (1.0, (_random_in_range(), _random_in_range())), 0.2,
                                         "Uniform weights and random offsets")
    ...
    def test_layout_relax_into_grid_random_offset_random_weight(self):
        self._run_layout_relax_into_grid(lambda: (np.random.rand(), (_random_in_range(), _random_in_range())), 0.2,
                                         "Random weights and offsets")
    ...
    def test_layout_relax_into_grid_random_offset_propoportinal_weight(self):
        ...
        """
        self._run_layout_relax_into_grid(_random_weight_proportional_offset, 0.2,
                                         "Random weights and proportional offsets")
    ...
    def _run_layout_relax_into_grid(self, weight_distance_generator_func, error_scalar, description):
        ...
        result = nornir_imageregistration.views.plot_layout(layout, title="A random graph.  Before relaxation",
                                                            PassFail=True)
        self.assertTrue(result)
        ...
        result = nornir_imageregistration.views.plot_layout(layout,
                                                            title=f"A regular grid.  After relaxation.\n{description}",
                                                            PassFail=True)
        self.assertTrue(result)
