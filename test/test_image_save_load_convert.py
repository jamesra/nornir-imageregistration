...
class TestImageSaveLoadConvert(setup_imagetest.ImageTestBase):
    ...
    def RunConvertImageTest(self, input_image_fullpath: str):
        ...
        leveled_image = nornir_imageregistration.core._ConvertSingleImage(input_image_fullpath,
                                                                          MinMax=(min_val, max_val), Bpp=Bpp)

        #         self.assertTrue(nornir_imageregistration.ShowGrayscale([original_image,leveled_image],
        #                                                                PassFail=True,
        #                                                                title="The original image"))

        inverted_leveled_image = nornir_imageregistration.core._ConvertSingleImage(input_image_fullpath,
                                                                                   MinMax=(min_val, max_val), Bpp=Bpp,
                                                                                   Invert=True)
        gamma_lowered_image = nornir_imageregistration.core._ConvertSingleImage(input_image_fullpath,
                                                                                MinMax=(min_val, max_val), Bpp=Bpp,
                                                                                Gamma=0.7)
        gamma_raised_image = nornir_imageregistration.core._ConvertSingleImage(input_image_fullpath,
                                                                               MinMax=(min_val, max_val), Bpp=Bpp,
                                                                               Gamma=1.3)
        ...
        self.RunSaveLoadImageTest_BppOnly(inverted_leveled_image,
                                          os.path.join(self.TestOutputPath, 'inverted_leveled.png'), Bpp)
        self.RunSaveLoadImageTest_BppOnly(gamma_lowered_image, os.path.join(self.TestOutputPath, 'gamma_raised.png'),
                                          Bpp)
        self.RunSaveLoadImageTest_BppOnly(gamma_raised_image, os.path.join(self.TestOutputPath, 'gamma_lowered.png'),
                                          Bpp)
