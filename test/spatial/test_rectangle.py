...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.OverlapRects = {"BottomHalfYOverlapA1": spatial.Rectangle.CreateFromPointAndArea((0, 0), (5, 5)),
                             ...
                             "BottomLeftQuarterOverlapA1": spatial.Rectangle.CreateFromPointAndArea((-5, -2.5),
                                                                                                    (10, 5)),
                             }
    ...
    def testOverlaps(self):
        ...
        self.assertEqual(spatial.Rectangle.overlap(self.ARects["A1"], self.ARects["A1"]), 1.0,
                         "Identical rectangles should have 1.0 overlap")
        self.assertEqual(spatial.Rectangle.overlap(self.ARects["A1"], self.ARects["A1BoundsDefined"]), 1.0,
                         "Identical rectangles should have 1.0 overlap")

        self.assertEqual(spatial.Rectangle.overlap(self.ARects["A1"], self.BRects["B1"]), 0,
                         "Non-overlapping rectangles should have 0 overlap")

        self.assertEqual(spatial.Rectangle.overlap(self.ARects["A1"], self.OverlapRects["BottomHalfYOverlapA1"]), 0.5,
                         "Expected 50% overlap with BottomHalfYOverlapA1")
        self.assertEqual(spatial.Rectangle.overlap(self.ARects["A1"], self.OverlapRects["TopHalfYOverlapA1"]), 0.5,
                         "Expected 50% overlap with TopHalfYOverlapA1")

        self.assertEqual(spatial.Rectangle.overlap(self.ARects["A1"], self.OverlapRects["LeftHalfXOverlapA1"]), 0.5,
                         "Expected 50% overlap with LeftHalfXOverlapA1")
        self.assertEqual(spatial.Rectangle.overlap(self.ARects["A1"], self.OverlapRects["RightHalfXOverlapA1"]), 0.5,
                         "Expected 50% overlap with RightHalfXOverlapA1")

        self.assertEqual(spatial.Rectangle.overlap(self.ARects["A1"], self.OverlapRects["TopRightQuarterOverlapA1"]),
                         0.25, "Expected 50% overlap with LeftHalfXOverlapA1")
        self.assertEqual(spatial.Rectangle.overlap(self.ARects["A1"], self.OverlapRects["BottomLeftQuarterOverlapA1"]),
                         0.25, "Expected 50% overlap with RightHalfXOverlapA1")
    ...
    def EnumerateOverlappingRectangles(self, rect_list):
        ...
        for (A, B) in rset.EnumerateOverlapping():
            ...
            OverlapSets[A].add(B)
            OverlapSets[B].add(A)
            self.assertTrue(spatial.Rectangle.Intersect(rect_list[A], rect_list[B]) is not None,
                            "Overlapping rectangles do not overlap")
        ...
        for (A, overlap_set) in OverlapSets.items():
            ...
            for B in non_overlap_set:
                if A != B:
                    self.assertFalse(spatial.Rectangle.contains(rect_list[A], rect_list[B]),
                                     "%d - %d: Non-overlapping rectangles overlap" % (A, B))
    ...
    def CompareGroups(self, DictA, DictB):
        ...
        for KeyA, A in list(DictA.items()):
            for keyB, B in list(DictB.items()):
                self.assertFalse(spatial.Rectangle.contains(A, B),
                                 "Non overlapping rectangles %s - %s should not contain each other" % (KeyA, keyB))
    ...
    def CompareCombinations(self, DictA):
        ...
        for (iOne, iTwo) in itertools.combinations(list(DictA.keys()), 2):
            ...
            self.assertTrue(spatial.Rectangle.contains(RectOne, RectTwo),
                            "Rectangles %s - %s from same group should overlap" % (iOne, iTwo))
