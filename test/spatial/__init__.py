...
def rectangles(draw, minCount=0, maxCount=128, shapeRange=None, positionRange=None):
    ...
    if shapeRange is None:
        shapeRange = (0, (positionRange[1] - positionRange[0]) + 1) #Default to a max shape that spans the entire range of origins
    ...
    points = draw(hypothesis.extra.numpy.arrays(np.float32, 
                    shape=(nRects,2), 
                    elements=hypothesis.strategies.floats(positionRange[0], positionRange[1], width=16), fill=hypothesis.strategies.nothing()))
    shapes = draw(hypothesis.extra.numpy.arrays(np.uint8,
                    shape=(nRects,2),
                    elements=hypothesis.strategies.integers(shapeRange[0], shapeRange[1]), fill=hypothesis.strategies.nothing()))
