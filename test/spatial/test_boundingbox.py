...
class Test(unittest.TestCase):
    ...
    def CompareGroups(self, DictA, DictB):
        ...
        for KeyA, A in list(DictA.items()):
            for keyB, B in list(DictB.items()):
                self.assertFalse(spatial.BoundingBox.contains(A, B),
                                 "Non overlapping boxes %s - %s should not contain each other" % (KeyA, keyB))
    ...
    def CompareCombinations(self, DictA):
        ...
        for (iOne, iTwo) in itertools.combinations(list(DictA.keys()), 2):
            ...
            self.assertTrue(spatial.BoundingBox.contains(RectOne, RectTwo),
                            "Boxes %s - %s from same group should overlap" % (iOne, iTwo))
