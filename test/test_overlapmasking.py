...
class TestOverlapMask(setup_imagetest.ImageTestBase):
    ...
    def testSquareOddOverlapMask(self):
        ...
        self.assertTrue(nornir_imageregistration.ShowGrayscale([mask],title="Square Overlap Mask with odd dimensions: 25% Min overlap, 75% max overlap", PassFail=True))
    ...
    def testSquareOddWidthOverlapMask(self):
        ...
        mask = nornir_imageregistration.GetOverlapMask(FixedImageSize, MovingImageSize, CorrelationImageSize, MinOverlap=0.25, MaxOverlap=0.75)
        np.testing.assert_equal(CorrelationImageSize, mask.shape, "Dimensions of output mask is incorrect")
        self.assertTrue(nornir_imageregistration.ShowGrayscale([mask],title="Square Overlap Mask with odd width dimension: 25% Min overlap, 75% max overlap", PassFail=True))
    ...
    def testSquareOddHeightOverlapMask(self):
        ...
        mask = nornir_imageregistration.GetOverlapMask(FixedImageSize, MovingImageSize, CorrelationImageSize, MinOverlap=0.25, MaxOverlap=0.75)
        np.testing.assert_equal(CorrelationImageSize, mask.shape, "Dimensions of output mask is incorrect")
        self.assertTrue(nornir_imageregistration.ShowGrayscale([mask],title="Square Overlap Mask with odd height dimension: 25% Min overlap, 75% max overlap", PassFail=True))
