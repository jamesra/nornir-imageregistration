...
class TestAssemble(setup_imagetest.ImageTestBase):
    ...
    def setUp(self):
        ...
        self.WarpedImagePath = os.path.join(self.ImportedDataPath,
    ...
    def test_TransformImageIdentity_GPU(self):
    ...
    def CallTransformImage(self, imageDim):
        ...
        outputImage = assemble.TransformImage(identity_transform, np.array([Height, Width]), warpedImage,
                                              CropUndefined=False)
    ...
    def test_SourceImageToTargetSpace_IdentityTransform_GPU(self):
        if nornir_imageregistration.HasCupy():
            '''Transform an image with the identity transform and ensure the output matches the input'''

            arecord = AlignmentRecord(peak=(0, 0), weight=100, angle=0.0)

            nornir_imageregistration.SetActiveComputationLib(nornir_imageregistration.ComputationLib.cupy)

            (fixedImage, warpedImage, transformedImage) = self.Run_SourceImageToTargetSpaceTransform(
                self.WarpedImagePath,
                self.WarpedImagePath,
                arecord,
                (0, 0),
    ...
    def Run_SourceImageToTargetSpaceTransform(self, FixedImagePath: str,
        ...
        transform = alignment_record.ToImageTransform(source_image_shape=warpedImage.shape,
                                                      target_image_shape=fixedImage.shape)
...
class TestStosFixedMovingAssemble(setup_imagetest.ImageTestBase):
    ...
    def RunStosAssemble(self, stosFullPath):
        ...
        self.assertEqual(nornir_imageregistration.GetImageSize(self.FixedImagePath)[0],
                         nornir_imageregistration.GetImageSize(OutputPath)[0])
        self.assertEqual(nornir_imageregistration.GetImageSize(self.FixedImagePath)[1],
